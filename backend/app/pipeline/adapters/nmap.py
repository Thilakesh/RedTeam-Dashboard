...
class NmapStage:
    ...
    _MAX_HOSTS = 50
    # Bounded concurrency instead of one host at a time — sequential scanning
    # of up to _MAX_HOSTS hosts (each with its own 120s subprocess timeout)
    # has a worst case of 100 minutes, which blows past the job's overall time
    # budget on large targets. Scanning several hosts at once keeps the same
    # per-host timeout but cuts the worst-case wall time roughly by this factor.
    _CONCURRENCY = 8

    async def _scan_host(
        self, binary: str, host: str, ports: list[int]
    ) -> list[AssetRecord]:
    ...
    async def execute(self, ctx: StageContext) -> list[AssetRecord]:
        ...
        async def bounded(host: str) -> list[AssetRecord]:
            async with semaphore:
                return await self._scan_host(binary, host, host_ports[host])

        results = await asyncio.gather(*(bounded(h) for h in hosts))
        return [rec for host_records in results for rec in host_records]
